...
Instruction load_instruction(const mlx_delegate::Instruction* fb_instr) {
  ...
  switch (op_type) {
    ...
    case mlx_delegate::OpNode_BitwiseInvertNode: {
      auto fb = fb_instr->op_as_BitwiseInvertNode();
      if (!fb) {{
        throw std::runtime_error("FlatBuffer op_type/payload mismatch for {class_name}");
      }}
      BitwiseInvertNode node;
      node.x = convert_tid(fb->x());
      node.out = convert_tid(fb->out());
      instr.op = OpCode::BITWISE_INVERT;
      instr.node = std::move(node);
      break;
    }
    ...
    case mlx_delegate::OpNode_BitwiseAndNode: {
      auto fb = fb_instr->op_as_BitwiseAndNode();
      if (!fb) {{
        throw std::runtime_error("FlatBuffer op_type/payload mismatch for {class_name}");
      }}
      BitwiseAndNode node;
    ...
    case mlx_delegate::OpNode_RollNode: {
      auto fb = fb_instr->op_as_RollNode();
      if (!fb) {{
        throw std::runtime_error("FlatBuffer op_type/payload mismatch for {class_name}");
      }}
      RollNode node;
      ...
      node.out = convert_tid(fb->out());
      if (fb->shift()) {
        for (size_t i = 0; i < fb->shift()->size(); ++i) {
          node.shift.push_back(convert_int_or_vid(fb->shift()->Get(static_cast<flatbuffers::uoffset_t>(i))));
        }
      }
      node.axes = to_vector(fb->axes());
      instr.op = OpCode::ROLL;
      instr.node = std::move(node);
      break;
    }
